...
from pulp_layer_placement import (  # noqa: E402
    optimize_placement,
    predict_latency_ms,
    EFF_WEIGHT, EFF_ACT, LAUNCH_US, KERNELS_PER_LAYER,
    PCIE_BW_PINNED, PCIE_BW_UNPINNED, MEMCPY_BW,
    HANDOFF_FIXED_US, TOK_ROUNDTRIP_US,
)

DEFAULT_CSV = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    "..", "..", "oracle", "40gb", "20_10_10", "vicuna_7B.csv",
)
    ...
    with open(path) as f:
        ...
        for r in reader:
            if r[COL_STATUS] != "ok":
                continue
            rows.append({
                "split": (int(r[COL_SPLIT0]), int(r[COL_SPLIT1]), int(r[COL_SPLIT2])),
...
def run_q1(rows, out_path=None):
    ...
    print(f"\n  rows: {s['n']}")
    print(f"  error%:  min={s['min']:7.1f}  p25={s['p25']:7.1f}  "
    ...
    for n in sorted({r["nmb"] for r in rows}):
        ...
        print(f"    N={n:2d}  rows={len(sub):4d}  median={statistics.median(sub):7.1f}%")

    print("\n  by batch_size:")
    for b in sorted({r["batch"] for r in rows}):
        sub = [r["err"] for r in rows if r["batch"] == b]
        print(f"    batch={b:3d} rows={len(sub):4d}  median={statistics.median(sub):7.1f}%")

    # Drift across N tells you whether the overlap law itself is off, separately
    # from the overall scale.
    meds = [statistics.median([r["err"] for r in rows if r["nmb"] == n])
            for n in sorted({r["nmb"] for r in rows})]
    if len(meds) > 1:
        drift = meds[-1] - meds[0]
        ...
        if abs(drift) < 3:
            ...
            print("  -> model too OPTIMISTIC at high N (real overlap is worse than ideal GPipe)")
        else:
            print("  -> model too PESSIMISTIC at high N (real overlap is better than modelled)")

    if out_path:
        with open(out_path, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["split", "batch_size", "microbatch_size", "num_microbatches",
                        "predicted_ms", "measured_ms", "error_pct"])
            for r in rows:
                w.writerow([
                    "/".join(str(x) for x in r["split"]),
                    r["batch"], r["mb"], r["nmb"],
                    round(r["pred"], 1), round(r["meas"], 1), round(r["err"], 1),
                ])
...
def run_q2(rows):
    ...
    for batch in sorted({r["batch"] for r in rows}):
        ...
        if pinned:
            print(f"    NOTE: at the memory limit on slice(s) {pinned} -- "
        ...
        for n in sorted({r["nmb"] for r in rows if r["batch"] == batch}):
            ...
            if not hit:
                ...
                print(f"    N={n}: NOT in sweep. {len(sub)} splits measured; "
            ...
            print(f"    N={n}: measured {h['meas']:9.0f} ms | rank {rank:3d}/{len(sub)} | "
...
def main():
    ...
    print(f"  EFF_WEIGHT={EFF_WEIGHT}  EFF_ACT={EFF_ACT}  "
          f"LAUNCH_US={LAUNCH_US}  KERNELS_PER_LAYER={KERNELS_PER_LAYER}")
    print(f"  PCIE_BW_PINNED={PCIE_BW_PINNED:.2e}  "
